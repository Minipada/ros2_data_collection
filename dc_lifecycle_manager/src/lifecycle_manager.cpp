...
// SPDX-FileCopyrightText: 2022 Samsung Research America
  ...
  : Node("lifecycle_manager", options), diagnostics_updater_(this)
{
  ...
  declare_parameter("transitions", rclcpp::PARAMETER_STRING_ARRAY);
  declare_parameter("project", rclcpp::ParameterValue("DC"));
  ...
  transitions_ = get_parameter("transitions").as_string_array();
...
void LifecycleManager::managerCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
...
void LifecycleManager::isActiveCallback(const std::shared_ptr<rmw_request_id_t> /*request_header*/,
...
void LifecycleManager::CreateActiveDiagnostic(diagnostic_updater::DiagnosticStatusWrapper& stat)
{
  if (system_active_)
  {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::OK, project_ + " is active");
  }
  else
  {
    stat.summary(diagnostic_msgs::msg::DiagnosticStatus::ERROR, project_ + " is inactive");
  }
}
...
std::uint8_t LifecycleManager::getTransition(const std::string& transition)
{
  if (transition.compare("configure") == 0)
  {
    return Transition::TRANSITION_CONFIGURE;
  }
  else if (transition.compare("activate") == 0)
  {
...
void LifecycleManager::createBondTimer()
{
  ...
  bond_timer_ =
      this->create_wall_timer(200ms, std::bind(&LifecycleManager::checkBondConnections, this), callback_group_);
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(dc_lifecycle_manager::LifecycleManager)
