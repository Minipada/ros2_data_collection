...
void FlbS3::initFlbOutputPlugin()
{
  ...
  out_ffd_ = flb_output(ctx_, "s3", NULL);
  flb_output_set(ctx_, out_ffd_, "match", destination_name_.c_str(), NULL);

  flb_output_set(ctx_, out_ffd_, "region", region_.c_str(), NULL);
  ...
  flb_output_set(ctx_, out_ffd_, "total_file_size", total_file_size_.c_str(), NULL);
  ...
  flb_output_set(ctx_, out_ffd_, "use_put_object", dc_util::boolToString(use_put_object_), NULL);
  flb_output_set(ctx_, out_ffd_, "role_arn", role_arn_.c_str(), NULL);
  flb_output_set(ctx_, out_ffd_, "endpoint", endpoint_.c_str(), NULL);
  flb_output_set(ctx_, out_ffd_, "sts_endpoint", sts_endpoint_.c_str(), NULL);
  flb_output_set(ctx_, out_ffd_, "canned_acl", canned_acl_.c_str(), NULL);
  flb_output_set(ctx_, out_ffd_, "compression", compression_.c_str(), NULL);
  flb_output_set(ctx_, out_ffd_, "content_type", content_type_.c_str(), NULL);
  flb_output_set(ctx_, out_ffd_, "send_content_md5", dc_util::boolToString(send_content_md5_), NULL);
  ...
  flb_output_set(ctx_, out_ffd_, "preserve_data_ordering", dc_util::boolToString(preserve_data_ordering_), NULL);
  flb_output_set(ctx_, out_ffd_, "storage_class", storage_class_.c_str(), NULL);
  flb_output_set(ctx_, out_ffd_, "retry_limit", retry_limit_.c_str(), NULL);
  flb_output_set(ctx_, out_ffd_, "external_id", external_id_.c_str(), NULL);
