...
`error_code` is `FollowWaypoints::Result.error_code` verbatim -- in `nav2_msgs`, its only non-zero values are `UNKNOWN` (600) and `TASK_EXECUTOR_FAILED` (601); `missed_waypoints[].error_code` is nav2's per-waypoint `error_code` from the same table. nav2's `WaypointStatus` (new since Lyrical) also carries the waypoint's pose, a status enum and a free-text `error_msg` -- none part of this schema (the `index` key keeps the Jazzy-line Record shape downstream consumers already receive); `reason`/`error_msg` above are always the mission-level `FollowWaypoints::Result.error_msg`, not anything per-waypoint.
