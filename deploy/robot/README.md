...
podman run --rm -it --name dc_robot_dc_ros --network dc_robot_net \
  ...
  ghcr.io/minipada/ros2_data_collection/dc-ros:rolling \
...
podman run --rm -it --name dc_robot_dc_uploader --network dc_robot_net \
  ...
  ghcr.io/minipada/ros2_data_collection/dc-uploader:rolling
```
...
#    branch already publishes, tagged :rolling — ghcr.io tags follow the branch name
#    here, never :latest, the same way ROS's own images are tagged by distro codename
#    (ros:rolling-ros-base), never ros:latest. --entrypoint true skips actually launching
...
#    changes.
podman run --rm --entrypoint true ghcr.io/minipada/ros2_data_collection/dc-ros:rolling
podman run --rm --entrypoint true ghcr.io/minipada/ros2_data_collection/dc-uploader:rolling
...
#    after it is fast.
rm -f /tmp/dc-ros.tar && podman save ghcr.io/minipada/ros2_data_collection/dc-ros:rolling -o /tmp/dc-ros.tar && k3d image import /tmp/dc-ros.tar -c dc-robot-dev
rm -f /tmp/dc-uploader.tar && podman save ghcr.io/minipada/ros2_data_collection/dc-uploader:rolling -o /tmp/dc-uploader.tar && k3d image import /tmp/dc-uploader.tar -c dc-robot-dev

# 4. Run the Pod. `k3d/kustomization.yaml` overlays kubernetes/robot-pod.yaml: pins
#    both images to :rolling instead of the base manifest's :latest, and sets
