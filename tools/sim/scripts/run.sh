...
#   DC_SIM_WORLD            "" (default) for the real qrcodes.world warehouse, or "small"
#                           for dc_simulation/worlds/qrcodes_small.world -- an 8x12 m,
#                           4-station room generated by
#                           dc_simulation/tools/gen_small_world.py. The real warehouse
#                           runs at a 0.11-0.20 real-time factor (dc_simulation/README.md),
#                           which occasionally leaves Nav2's bringup right at the edge of
#                           the `nav` stage's activation timeout; the small world proves
#                           the same pipeline (spawn, localization, one drive-to-goal, one
#                           QR read) without the warehouse's 239 props. ci.yaml's PR-gating
#                           `sim` job uses it; the daily full-pass job (sim.yaml) does not
#                           -- it exists specifically to exercise the real demo.
...
#                           QR-coded pallet in the real warehouse (DC_SIM_WORLD=small has
#                           only 4 stations total), which the daily job runs.
...
case "$WORLD" in
  "")
    TOTAL_STATIONS=60
    WAYPOINT_EXEC=qrcodes_waypoint_follower
    WAREHOUSE_LAUNCH_ARGS=""
    QRCODES_LAUNCH_ARGS=""
    GROUND_TRUTH_ENV=""
    ;;
  small)
    TOTAL_STATIONS=4
    WAYPOINT_EXEC=qrcodes_small_waypoint_follower
    # Spawn/world-to-map values are dc_simulation/tools/gen_small_world.py's own SPAWN
    # constant -- world frame == map frame there, so the same numbers serve both.
    WAREHOUSE_LAUNCH_ARGS="world:=/root/ws/install/dc_simulation/share/dc_simulation/worlds/qrcodes_small.world x_pose:=-1.07 y_pose:=-4.3 z_pose:=0.01 yaw:=1.570796"
    QRCODES_LAUNCH_ARGS="world:=/root/ws/install/dc_simulation/share/dc_simulation/worlds/qrcodes_small.world map:=/root/ws/install/dc_simulation/share/dc_simulation/maps/qrcodes_small.yaml nav2_params_file:=/root/ws/install/dc_demos/share/dc_demos/params/qrcodes_small_nav.yaml x_pose:=-1.07 y_pose:=-4.3 z_pose:=0.01 yaw:=1.570796"
    GROUND_TRUTH_ENV="DC_SIM_WORLD_FILE=/root/ws/install/dc_simulation/share/dc_simulation/worlds/qrcodes_small.world DC_SIM_WORLD_TO_MAP=0,0"
    ;;
  *)
    fail "unknown DC_SIM_WORLD '$WORLD' (want: '' or 'small')" ;;
esac
...
if has_stage sim; then
  ...
  rbg "$SOURCE && ros2 launch dc_simulation warehouse.launch.py headless:=True $WAREHOUSE_LAUNCH_ARGS > /tmp/warehouse.log 2>&1"
  ...
  wait_for "the robot to spawn" 600 \
    'source /opt/ros/${ROS_DISTRO:-rolling}/setup.bash && gz model --list 2>/dev/null | grep -q turtlebot3_waffle' \
...
if has_stage waypoints || has_stage detect; then
  ...
  log "running $WAYPOINT_EXEC until it reaches waypoint $WAYPOINTS/$TOTAL_STATIONS"
  rbg "$SOURCE && ros2 run dc_demos $WAYPOINT_EXEC > /tmp/waypoints.log 2>&1; echo EXIT=\$? >> /tmp/waypoints.log"
  ...
  while [ "$waited" -lt "$WAYPOINT_TIMEOUT" ]; do
    ...
    if [ "$reached" -ge "$WAYPOINTS" ]; then
      log "reached waypoint $reached/$TOTAL_STATIONS after ${waited}s"
      break
    fi
  ...
  [ "$reached" -ge "$WAYPOINTS" ] || fail "only reached waypoint $reached/$TOTAL_STATIONS in ${WAYPOINT_TIMEOUT}s (wanted $WAYPOINTS)"
